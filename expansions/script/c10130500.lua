...
function cod.cfilter(c)
	...
	return c:IsLocation(LOCATION_PZONE) and c:IsAbleToDeck()
		and Duel.IsExistingMatchingCard(cod.scfilter,c:GetControler(),LOCATION_DECK,0,1,nil,lscale,rscale,c:GetCode())
end
function cod.scfilter(c,lscale,rscale,code)
	return c:IsType(TYPE_PENDULUM) and (c:GetLeftScale()==lscale or c:GetRightScale()==rscale) and not c:IsCode(code) and not c:IsForbidden()
end
...
function cod.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local lscale,rscale=tc:GetLeftScale(),tc:GetRightScale()
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc,nil,2,REASON_EFFECT)~=0 and Duel.CheckLocation(tp,LOCATION_SZONE,seq) then
		...
		local g=Duel.SelectMatchingCard(tp,cod.scfilter,tp,LOCATION_DECK,0,1,1,nil,lscale,rscale,tc:GetCode())
